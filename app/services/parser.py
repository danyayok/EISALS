...
class EISParser:
    ...
    def _sync_fetch(self, url: str) -> str | None:
        ...
        try:
            with urlopen(request, timeout=30) as response:
                ...
                if status != 200:
                    logger.warning("EIS status code %s for %s", status, url)
                    return None
                content = response.read()
                return content.decode("utf-8", errors="ignore")
        except URLError as exc:
            logger.warning("EIS request failed for %s: %s", url, exc)
            return None
    ...
    async def parse_cards(self, base_link: str, pages: int = 1) -> list[dict[str, Any]]:
        ...
        for page in range(1, pages + 1):
            url = str(URL(f"{self.BASE_URL}{base_link}").update_query(recordsPerPage="_50", pageNumber=str(page)))
            ...
            for card in cards:
                header_node = card.find("div", class_="registry-entry__header")
                header_text = self._clean_text(header_node.get_text(" ", strip=True) if header_node else None)
